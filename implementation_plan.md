...
### Component Overview
The application is a full-stack Club Management Platform (Node.js/Express backend + ES6 SPA frontend) deployed for Pragati University / Pragati Engineering College. It serves 5 distinct actor roles: Director (Academics), Faculty Coordinator, Club Admin / Club Lead, Student, and Guest. The system manages institutional club charters, memberships, attendance records with QR tokens, accredited certificates, and audit logs.
...
| `POST /api/certificates/issue` | JSON REST Body | Untrusted | Strict check: only Director (Academics) or assigned Faculty Coordinator (`certificates.approve`) |
...
| `POST /api/admin/users/*` | JSON REST Body | Untrusted | Strict check: caller must have `users.create` / `roles.assign` (Director (Academics) only) |
| `GET /api/audit-logs` | Query / Route | Untrusted | Role check: Director (Academics) gets full logs; Faculty Coordinator gets assigned club logs; Students/Guests blocked (403) |
| `POST /api/settings/*` | JSON REST Body | Untrusted | Strict check: `settings.manage` (Director (Academics) only) |
...
| Audit Logs | Mutating endpoints | Express DB | Restricted to Director (Academics) (full) and Faculty Coordinator (scoped); hidden from students/guests |
...
|---|---|---|
| User role modification | `POST /api/admin/users/update-role` | Requires `roles.assign` (Director (Academics) only) |
| Club deletion/approval | `POST /api/admin/clubs/:id/*` | Requires `clubs.approve` / `clubs.delete` (Director (Academics) only) |
...
| Settings management | `POST /api/settings/*` | Requires `settings.manage` (Director (Academics) only) |
